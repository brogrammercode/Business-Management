...
class BusinessCubit extends Cubit<BusinessState> {
  ...
  Future<void> initBusinessSubscription() async {
    try {
      ...
      _businessSubscription = _repo.getBusinesses().listen((businesses) {
        emit(state.copyWith(
            businesses: businesses, getBusinessStatus: StateStatus.success));
      });
    } catch (e) {
  ...
  Future<bool> addBusiness({
    ...
    try {
      ...
      final success =
          await _repo.addBusiness(business: business, avatar: avatar);
  ...
  Future<void> updateMyBusinessParams(
          {required List<MyBusinessParams> myBusiness}) async =>
      emit(state.copyWith(myBusiness: myBusiness));
